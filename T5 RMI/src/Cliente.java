...
public class Cliente {
    ...
    public static void main(String[] args) throws RemoteException, MalformedURLException, NotBoundException {
        // Leer direcciones ip locales
        if (args.length != 3) {
            System.err.println("Se requieren las ip como argumentos de ejecución");
            System.exit(1);
        }
        ...
        // Búsqueda de Objetos
        Remoto[] objetos = new Remoto[3];

        for (int i = 0; i < args.length; i++)
            objetos[i] = (Remoto) Naming.lookup("rmi://" + args[i] + "/prueba");
        ...
        double[][] C1 = objetos[0].multiplica_matrices(A1,B1);
        double[][] C2 = objetos[0].multiplica_matrices(A1,B2);
        double[][] C3 = objetos[0].multiplica_matrices(A1,B3);
        double[][] C4 = objetos[1].multiplica_matrices(A2,B1);
        double[][] C5 = objetos[1].multiplica_matrices(A2,B2);
        double[][] C6 = objetos[1].multiplica_matrices(A2,B3);
        double[][] C7 = objetos[2].multiplica_matrices(A3,B1);
        double[][] C8 = objetos[2].multiplica_matrices(A3,B2);
        double[][] C9 = objetos[2].multiplica_matrices(A3,B3);
